...
Branch and foliage biomass are calculated independently from
DBH-dependent ratios:
...
For either branch or foliage, the ratio is interpolated as:
...
``` math
R(D)=\frac{(D-40)l+(70-D)m}{30}, \quad 40<D\le70
```
...
where $`s`$, $`m`$, and $`l`$ are the published small-, medium-, and
large-tree ratios.
